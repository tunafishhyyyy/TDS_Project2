...
from config import (
    OPENAI_API_KEY, DEFAULT_MODEL, TEMPERATURE, MAX_TOKENS,
    VECTOR_STORE_TYPE, EMBEDDING_MODEL, CHAIN_TIMEOUT, MAX_RETRIES
)
...
class DataAnalysisChain(Chain, BaseWorkflow):
    ...
    def __init__(self, model_name: str = DEFAULT_MODEL, temperature: float = TEMPERATURE, **kwargs):
        ...
        object.__setattr__(self, 'chain', LLMChain(llm=self.llm, prompt=self.prompt_template))
...
class CodeGenerationChain(BaseWorkflow):
    ...
    def _create_code_prompt(self) -> PromptTemplate:
        ...
        template = """You are a Python data analysis expert. Generate clean, well-documented Python code based on the following requirements:
...
"""
        ...
        return PromptTemplate(
            input_variables=["task_description", "data_context", "libraries", "output_format"],
            template=template
        )
...
"""
        ...
        return PromptTemplate(
            input_variables=["analysis_results", "data_summary", "key_findings", "audience"],
            template=template
        )
